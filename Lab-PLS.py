...
DPath='E:/data/winddb/' # '/Users/harbes/PycharmProjects/data/winddb/' #
#indicators_all=pd.read_pickle(DPath+'indicators_all').replace([np.inf, -np.inf], np.nan)
freq='M'
indicators_all=pd.read_pickle(DPath + 'predictors_all_' + freq).replace(np.inf,np.nan)
...
'''
indi_standardized=indicators_all.groupby(level=0).apply(lambda x:(x-x.mean())/x.std())
